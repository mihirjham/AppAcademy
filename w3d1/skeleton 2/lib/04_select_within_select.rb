...
def neighbors_of_b_countries
  ...
  execute(<<-SQL)
    ...
    WHERE
      b.name = 'Belize' OR b.name = 'Belgium'
  SQL
end
...
def population_constraint
  ...
  execute(<<-SQL)
    ...
    FROM
      countries a
    JOIN
      countries b ON a.population > b.population
    JOIN
      countries c ON a.population < c.population
    WHERE
      b.name = 'Canada' AND c.name = 'Poland'
  SQL
end
